...
export async function creationOfContact(authToken: string): Promise<{ createdContact: string; responseData: string; responseStatus: string; contactId: string}> {
  try {
    ...
    const contactId: string = response.data._id;
    ...
    return { createdContact, responseData, responseStatus, contactId };
